"""Tests for the claude_chat plugin (port of legacy adapter)."""
...
def _new_plugin() -> ClaudeChatPlugin:
    """Build a ClaudeChatPlugin with the in-tree manifest."""
    from phdb.core.plugin.manifest import load_manifest

    manifest_path = Path("src/phdb/plugins/claude_chat/plugin.toml").resolve()
    manifest = load_manifest(manifest_path)
    return ClaudeChatPlugin(manifest)


class TestConversationsIngest:
    def test_row_count(self, migrated_db: Path, test_settings: Settings) -> None:
        test_settings.db_path = migrated_db
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            report = plugin.run(CONVOS, conn, test_settings)
        assert report.rows_yielded == EXPECTED_CONVO_ROWS
        assert report.rows_inserted == EXPECTED_CONVO_ROWS
        assert report.rows_skipped == 0

    def test_threads_created(self, migrated_db: Path, test_settings: Settings) -> None:
        test_settings.db_path = migrated_db
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            report = plugin.run(CONVOS, conn, test_settings)
    ...
    def test_kind_breakdown(self, migrated_db: Path, test_settings: Settings) -> None:
        test_settings.db_path = migrated_db
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            plugin.run(CONVOS, conn, test_settings)
    ...
    def test_tool_rows_marked_bulk(self, migrated_db: Path, test_settings: Settings) -> None:
    ...
    def test_role_assignment(self, migrated_db: Path, test_settings: Settings) -> None:
        test_settings.db_path = migrated_db
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            plugin.run(CONVOS, conn, test_settings)
    ...
    def test_thread_nodes_created(self, migrated_db: Path, test_settings: Settings) -> None:
        test_settings.db_path = migrated_db
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            plugin.run(CONVOS, conn, test_settings)
    ...
    def test_dedup_on_rerun(self, migrated_db: Path, test_settings: Settings) -> None:
        """Re-ingesting the same export must INSERT OR IGNORE everything."""
        test_settings.db_path = migrated_db
        with connect(migrated_db) as conn:
            r1 = _new_plugin().run(CONVOS, conn, test_settings)
        with connect(migrated_db) as conn:
            r2 = _new_plugin().run(CONVOS, conn, test_settings)
        assert r1.rows_inserted == EXPECTED_CONVO_ROWS
        assert r2.rows_inserted == 0
        assert r2.rows_skipped == r2.rows_yielded == EXPECTED_CONVO_ROWS

    def test_attachments_extracted(self, migrated_db: Path, test_settings: Settings) -> None:
    ...
    def test_sender_addresses(self, migrated_db: Path, test_settings: Settings) -> None:
...
class TestMemoriesIngest:
    def test_basic(self, migrated_db: Path, test_settings: Settings) -> None:
        test_settings.db_path = migrated_db
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            report = plugin.run(MEMORIES, conn, test_settings)
            count = conn.execute(
...
class TestUsersIngest:
    def test_basic(self, migrated_db: Path, test_settings: Settings) -> None:
        ...
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            report = plugin.run(USERS, conn, test_settings)
            row = conn.execute(
...
class TestProjectIngest:
    def test_definition_plus_docs(self, migrated_db: Path, test_settings: Settings) -> None:
        test_settings.db_path = migrated_db
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            report = plugin.run(PROJECT, conn, test_settings)
    ...
    def test_project_thread_keyed(self, migrated_db: Path, test_settings: Settings) -> None:
        test_settings.db_path = migrated_db
        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            plugin.run(PROJECT, conn, test_settings)
            row = conn.execute(
...
class TestDedupAcrossOverlappingExports:
    ...
    def test_overlap_dedups(self, migrated_db: Path, test_settings: Settings, tmp_path: Path) -> None:
        import json
        test_settings.db_path = migrated_db

        # First batch: first 3 conversations
        first = json.loads(CONVOS.read_text())[:3]
        # Use a name the plugin recognizes by parent_path/filename dispatch.
        ...
        second_dir.mkdir()
        second_file = second_dir / "conversations.json"
        second_file.write_text(CONVOS.read_text())

        plugin = _new_plugin()
        with connect(migrated_db) as conn:
            r1 = plugin.run(first_file, conn, test_settings)
        with connect(migrated_db) as conn:
            r2 = plugin.run(second_file, conn, test_settings)
