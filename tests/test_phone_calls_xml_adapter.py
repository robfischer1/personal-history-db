"""Tests for the phone_calls_xml adapter."""
...
from phdb.adapters.phone_calls_xml import (
    PhoneCallsXmlAdapter,
    _epoch_ms_to_iso,
    _normalize_phone,
    _synthesize_body,
)
...
FIXTURE_XML = Path(__file__).parent / "fixtures" / "phone_calls_xml" / "calls.xml"
...
class TestPhoneCallsXmlIntegration:
    ...
    def test_idempotent_rerun(self, calls_db: Path, calls_settings: Settings) -> None:
        ...
        adapter2 = PhoneCallsXmlAdapter()
        with connect(calls_db) as conn:
            r2 = adapter2.run(FIXTURE_XML, conn, calls_settings)
