"""Apple Health Backup ingest logic — handles sidecar tables and row insertions."""
...
if TYPE_CHECKING:
    from phdb.formats.apple_health_backup import ParsedRecord, ParsedWorkout

def register_source_file(
    conn: sqlite3.Connection,
    source_path: Path,
    *,
    source_kind: str = "apple-health-backup",
...
def upsert_observation(
    ...
    cur = conn.execute(
        """INSERT OR IGNORE INTO observations (
            ...
            'Observation', ?, ?, ?, ?, 'self', ?, ?, ?,
            'apple-health-backup', ?, 1, 'apple-health-record', ?, ?
        )""",
        (
            f"apple-health-backup:rec:{rec.raw_hash[:16]}", rec.record_type_label,
            rec.subject, f"apple-health:{rec.source_name}", rec.start_date, rec.end_date,
...
def upsert_exercise_action(
    ...
    cur = conn.execute(
        """INSERT OR IGNORE INTO exercise_actions (
            ...
            'ExerciseAction', ?, ?, ?, ?, 'self', ?, ?, ?,
            'apple-health-backup', ?, 1, 'apple-health-workout', ?, ?
        )""",
        (
            f"apple-health-backup:wkt:{wkt.raw_hash[:16]}", wkt.activity_label,
            wkt.subject, f"apple-health:{wkt.source_name}", wkt.start_date, wkt.end_date,
