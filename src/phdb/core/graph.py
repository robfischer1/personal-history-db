"""Graph service — typed-edge triple store over the vault and corpus.

Moved here from ``phdb.triples`` as part of Phase 1 of the phdb Plugin
...
predicates (controlled edge-type vocabulary), triples (subject →
predicate → object with timestamps and provenance), and qualifiers
(reified key/value metadata on triples).

All public functions take ``conn: sqlite3.Connection`` as first arg —
the module is stateless; callers own connection lifecycle. The
``GraphService`` class is a thin convenience wrapper that binds a
connection so callers can write ``svc.add_triple(...)`` instead of
threading ``conn`` through every call.
    ...
    *,
    ...
    if len(wikilinks) == 1 and bare:
        ...
        if bare_start < wl_pos.start():
            obj_id = resolve_node(conn, wl, "concept")  # noqa: F841 — side effect: ensure node
            return add_triple(
...
class GraphService:
    """Thin connection-bound wrapper around the module-level functions.

    Phase 1 deliverable — gives plugins and facets a single ``svc.``
    handle to use instead of threading ``conn`` through every call.
    Module-level functions remain the canonical surface; the class adds
    no new behavior.
    """
    ...
    # Node + predicate
    def resolve_node(self, label: str, kind: str = "concept", **kwargs: Any) -> int | None:
        return resolve_node(self.conn, label, kind=kind, **kwargs)

    def resolve_node_for_wikilink(self, wikilink: str, *, create: bool = True) -> int | None:
        return resolve_node_for_wikilink(self.conn, wikilink, create=create)

    def get_predicate(self, name: str) -> dict[str, Any] | None:
        return get_predicate(self.conn, name)

    def list_predicates(self) -> list[dict[str, Any]]:
        return list_predicates(self.conn)

    # Triples
    def add_triple(
        self, subject: str | int, predicate: str | int,
        object_: str | int | None = None, **kwargs: Any,
    ) -> dict[str, Any]:
        return add_triple(self.conn, subject, predicate, object_, **kwargs)

    def add_qualifier(self, triple_id: int, key: str, value: str | None = None) -> int:
        return add_qualifier(self.conn, triple_id, key, value)

    def query_triples(self, **kwargs: Any) -> list[dict[str, Any]]:
        return query_triples(self.conn, **kwargs)

    def get_qualifiers(self, triple_id: int) -> list[dict[str, Any]]:
        return get_qualifiers(self.conn, triple_id)

    def node_neighborhood(self, node: str | int, *, depth: int = 1, limit: int = 50) -> dict[str, Any]:
        return node_neighborhood(self.conn, node, depth=depth, limit=limit)

    # Emission
    def emit_for_frontmatter(
        self, rel_path: str, frontmatter: dict[str, Any],
        *, provenance: str = "ai-emitted",
    ) -> int:
        return emit_for_frontmatter(self.conn, rel_path, frontmatter, provenance=provenance)

    # Stats
    def stats(self) -> dict[str, Any]:
        return triple_stats(self.conn)
