...
Moved here from ``phdb.embed_pipeline`` as part of Phase 1 of the phdb
Plugin Architecture plan. The legacy module re-exports from this
location.

Phase 1 refactor: the previously-hardcoded ``_EMBEDDABLE_TABLES`` list +
the document/articles/clippings branches are driven by the
``phdb.core.registry`` registry. Phase 3 will populate that registry
from plugin manifests; this module's surface stays the same.
"""
...
def get_embed_status(
    ...
    *,
    ...
    for tname, _ in reg.embeddable_communication_tables:
...
def run_embed_pipeline(
    ...
    *,
    ...
    for tname, schema_type in reg.embeddable_document_tables:
        if not _has_table(conn, tname):
            continue
        pending_sql = _DOCUMENT_PENDING_SQL.get(tname)
        if pending_sql is None:
            continue
        sources.append((pending_sql, tname, schema_type))
